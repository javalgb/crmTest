layui.use(['form', 'layer','upload','element'], function () {
    ...
    form.on("submit(addOrUpdateSaleChance)",function(obj){
        ...
        var url=ctx+"/foodDetails/save";

        //判断当前页面的隐藏域有没有id,有id做修改，否则添加操作
        if($("input[name=id]").val()){
            url=ctx+"/foodDetails/update"
        }
    ...
    $.ajax({
        type:"post",
        url:ctx+"/foodType/foodTypes",
        dataType:"json",
        success:function(data){
            //遍历
            for (var x in data) {
                $("#foodTypeName").append("<option  value='"+data[x].foodTypeName+"'>"+data[x].foodTypeName+"</option>");
    ...
    var uploadInst = upload.render({
        ...
        ,before: function(obj){
            ...
            obj.preview(function(index, file, result){
                ...
                $("input[name=foodImg]").val(file.name)
                console.log(index)
                console.log(file)
                console.log(result)
            });
